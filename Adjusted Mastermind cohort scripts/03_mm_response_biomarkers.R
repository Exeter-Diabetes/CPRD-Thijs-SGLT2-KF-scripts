...
## additionally find date of next uACR measurement post-baseline and date of new macroalbuminuria (acr >30mg/mmol)
...
codes = codesets$getAllCodeSetVersion(v = "31/10/2021")
  ...
  # Add in next eGFR and acr measurement
  
  analysis = cprd$analysis("all")
  
  egfr_long <- egfr_long %>% analysis$cached("patid_clean_egfr_medcodes")
  acr_temp <- acr_temp %>% analysis$cached("patid_clean_acr_from_separate_medcodes")
  acr_temp2 <- acr_temp2 %>% analysis$cached("patid_clean_acr_medcodes")
...
prev_acr <- baseline_biomarkers %>%
  ...
  mutate(
    preacr_confirmed = ifelse(testvalue >= 3 & (preacr_previous >= 3 & datediff(preacr_previous_date, preacrdate) <=7 & datediff(preacr_previous_date, preacrdate) >= -730 | preacr_next >= 3), TRUE, FALSE),
  ...
  filter(abs(drugdatediff) == min(abs(drugdatediff), na.rm = TRUE)) %>%  # Keep row with closest drugdatediff
  ungroup() %>%
  select(patid, dstartdate, drugclass, preacr_confirmed, preacr_previous, preacr_previous_date, preacr_next, preacr_next_date) %>%
...
## Join drug start dates with all longitudinal acr measurements, and only keep later acr measurements which are >30mg/mmol
## Checked and those with null acr do get dropped
