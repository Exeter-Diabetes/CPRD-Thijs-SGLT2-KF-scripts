...
meds <- c("ace_inhibitors",
          ...
          "loop_diuretics",
          "ksparing_diuretics",
          #"definite_genital_infection_meds", #remove as name is too long
...
for (i in meds) {
  ...
  drug_merge_tablename <- paste0("full_", i, "_drug_merge")
  interim_non_diab_meds_table <- paste0("non_dm_meds_im_", i)
...
non_diabetes_meds <- non_diabetes_meds %>%
  ...
  analysis$cached("nondiabetes_meds", indexes=c("patid", "dstartdate", "drugclass"))