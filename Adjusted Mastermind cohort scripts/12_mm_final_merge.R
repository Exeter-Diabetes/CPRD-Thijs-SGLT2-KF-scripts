...
##########LEAVE OUT BELOW - WILL CALCULATE RISK SCORES AFTER MULTIPLE IMPUTATION##################################################################################
...
#  mutate(sex2=ifelse(sex=="male", "male", ifelse(sex=="female", "female", NA))) %>%
#  
#  calculate_ckdpc_egfr60_risk(age=dstartdate_age, sex=sex2, black_eth=black_ethnicity, 
#                              egfr=preegfr, cvd=cvd, hba1c=prehba1c, insulin=INS, oha=oha, 
#                              ever_smoker=ever_smoker, hypertension=hypertension, bmi=prebmi, acr=uacr, 
#                              complete_acr=TRUE, 
#                              remote=TRUE) %>%
#  
...
#below are the scores without assuming acr 10mg/g if missing or 0.

#ckdpc_scores <- ckdpc_scores %>%
#  
#  mutate(sex2=ifelse(sex=="male", "male", ifelse(sex=="female", "female", NA))) %>%
#  
#  calculate_ckdpc_egfr60_risk(age=dstartdate_age, sex=sex2, black_eth=black_ethnicity, egfr=preegfr, 
#                              cvd=cvd, hba1c=prehba1c, insulin=INS, oha=oha, ever_smoker=ever_smoker, 
#                              hypertension=hypertension, bmi=prebmi, acr=uacr, 
#                              remote=TRUE) %>%
#  
#  analysis$cached(paste0(today, "_t2d_1stinstance_interim_ckd3"), indexes=c("patid", "dstartdate", "drugclass"))

##calculate risk score for 40% decline + exclude cases with variables outside range

#ckdpc_scores <- ckdpc_scores %>%
#  
#  mutate(sex2=ifelse(sex=="male", "male", ifelse(sex=="female", "female", NA))) %>%
#  
#  calculate_ckdpc_40egfr_risk(age=dstartdate_age, sex=sex2, egfr=preegfr, acr=uacr, 
#                              sbp=presbp, bp_meds=bp_meds, hf=predrug_heartfailure, 
#                              chd=chd, af=predrug_af, current_smoker=current_smoker, 
#                              ex_smoker=ex_smoker, bmi=prebmi, hba1c=prehba1c, oha=oha, 
#                              insulin=INS, remote=TRUE) %>%
#  
