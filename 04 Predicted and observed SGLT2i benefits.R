...
noncal_cohort_lowegfr <- noncal_cohort %>% filter(egfr_below_60 == T & macroalbuminuria == F)
...
risk_threshold_1 <- noncal_cohort[noncal_cohort$microalbuminuria == T & noncal_cohort$egfr_below_60 == F,]$ckdpc_40egfr_score_cal %>% quantile(risk_threshold) %>% as.numeric()
...
noncal_cohort <- noncal_cohort %>% mutate(
  ckd40_risk_group = ifelse(noncal_cohort$microalbuminuria == T & noncal_cohort$egfr_below_60 == F &
    ckdpc_40egfr_score_cal >= (risk_threshold_1), high_risk_cat, low_risk_cat),
...
## plot for eGFR >60 and uACR <3 ====

#fit unadjusted curve so we can get risk table with number at risk and number of events:

surv_noalb <- survfit(Surv(ckd_egfr40_censtime_yrs, ckd_egfr40_censvar) ~ studydrug2,
...
list_plots_adjusted[[1]] <- ggsurvplot(
  ...
  xlab = "",
  ylab = "40% decline in eGFR / ESKD",
  title = paste0(c("eGFR ≥60mL/min, uACR <3mg/mmol"), collapse=""),
  subtitle = " "
)
...
## plot for eGFR >60 and uACR 3-30 and risk 0-90% =====
...
list_plots_adjusted[[3]] <- ggsurvplot(
  ...
  xlab = "",
  ylab = "",
  title = paste0(c("eGFR ≥60mL/min, uACR 3-30mg/mmol"), collapse=""),
  subtitle = paste0(c(low_risk_cat))
)
...
## plot for eGFR >60 and uACR 3-30 and risk >90% ====
...
list_plots_adjusted[[5]] <- ggsurvplot(
  ...
  ylab = "",
  title = paste0(c("eGFR ≥60mL/min, uACR 3-30mg/mmol"), collapse=""),
  subtitle = paste0(c(high_risk_cat))
) 
...
## plot for eGFR 20-60 and uACR <3 ====

surv_lowegfr_noalb <- survfit(Surv(ckd_egfr40_censtime_yrs, ckd_egfr40_censvar) ~ studydrug2,
...
list_plots_adjusted[[2]] <- ggsurvplot(
  ...
  xlab = "Years",
  ylab = "40% decline in eGFR / ESKD",
  title = paste0(c("eGFR 20-60mL/min, uACR <3mg/mmol"), collapse=""),
  subtitle = " "
)
...
## plot for eGFR 20-60 and uACR 3-30 ====

surv_lowegfr_alb <- survfit(Surv(ckd_egfr40_censtime_yrs, ckd_egfr40_censvar) ~ studydrug2,
                                 data = noncal_cohort_lowegfr[noncal_cohort_lowegfr$.imp == 1 & 
...
list_plots_adjusted[[6]] <- ggsurvplot(
  ...
  tables.y.text.col = T,
  tables.theme = theme_cleantable(),
  title = paste0(c("Any eGFR, uACR ≥30mg/mmol"), collapse=""),
  subtitle = " "
)
...
# add risk group variable for cohort of people with eGFR ≥60mL/min
noncal_cohort <- noncal_cohort %>%
  mutate(risk_group = ifelse(albuminuria == F, "Preserved eGFR, no albuminuria", 
                             ifelse(ckd40_risk_group == low_risk_cat, "Preserved eGFR, microalbuminuria, 0-90% risk",
                                    "Preserved eGFR, microalbuminuria, ≥90% risk")),
         risk_group = factor(risk_group, levels=c("Preserved eGFR, no albuminuria", 
                                                  "Preserved eGFR, microalbuminuria, 0-90% risk",
                                                  "Preserved eGFR, microalbuminuria, ≥90% risk")))
...
## calculate ARR and NNT for macroalbuminuria (uACR >=30)
...
obs40_macroalb <- obs 
obs_v_pred40_macroalb <- pred40_macroalb %>% cbind(obs40_macroalb) %>%
  mutate(nnt_observed=1/surv_diff) %>% select(-se)


## calculate ARR and NNT for reduced eGFR (eGFR 20-60)
...
dpp4isu_events <- noncal_cohort_lowegfr %>%
  filter(studydrug2=="DPP4i/SU" & ckd_egfr40_censvar==1) %>%
  group_by(albuminuria) %>%
  summarise(`DPP4i/SU`=round(n()/n.imp,0))

sglt2i_events <- noncal_cohort_lowegfr %>%
  filter(studydrug2=="SGLT2i" & ckd_egfr40_censvar==1) %>%
  group_by(albuminuria) %>%
  summarise(SGLT2i=round(n()/n.imp,0))
...
nnt_observed_weighted <-   matrix(data = NA, nrow = 6, ncol = n.imp)
...
for (i in 1:n.imp) {
  ...
  surv_lowegfr_noalb <- survfit(Surv(ckd_egfr40_censtime_yrs, ckd_egfr40_censvar) ~ studydrug2,
                                data = noncal_cohort_lowegfr[noncal_cohort_lowegfr$.imp == i & 
                                                               noncal_cohort_lowegfr$albuminuria == F,],
                                weights = overlap2,
                                conf.type = "log", conf.int = 0.95)
  ...
  surv_macroalb <- survfit(Surv(ckd_egfr40_censtime_yrs, ckd_egfr40_censvar) ~ studydrug2,
                           data = noncal_cohort_macroalb[noncal_cohort_macroalb$.imp == i,],
                           weights = overlap2,
                           conf.type = "log", conf.int = 0.95)
  
  nnt_observed_weighted[1,i] <- summary(surv_noalb, times = 3)$surv[2]-summary(surv_noalb, times = 3)$surv[1]
  nnt_observed_weighted[2,i] <- summary(surv_alb_lowrisk, times = 3)$surv[2]-summary(surv_alb_lowrisk, times = 3)$surv[1]
  nnt_observed_weighted[3,i] <- summary(surv_alb_highrisk, times = 3)$surv[2]-summary(surv_alb_highrisk, times = 3)$surv[1]
  nnt_observed_weighted[4,i] <- summary(surv_lowegfr_noalb, times = 3)$surv[2]-summary(surv_lowegfr_noalb, times = 3)$surv[1]
  nnt_observed_weighted[5,i] <- summary(surv_lowegfr_alb, times = 3)$surv[2]-summary(surv_lowegfr_alb, times = 3)$surv[1]
  nnt_observed_weighted[6,i] <- summary(surv_macroalb, times = 3)$surv[2]-summary(surv_macroalb, times = 3)$surv[1]
}
...
nnt_table <- rbind(obs_v_pred40 %>% select(risk_group, 
...
table <- CreateTableOne(vars = vars, strata = "egfr_below_45", data = noncal_cohort_lowegfr[noncal_cohort_lowegfr$albuminuria == F,],
                        factorVars = factors, test = F)
