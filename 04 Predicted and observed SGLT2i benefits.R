...
high_risk_cat <- paste(c("Risk ≥ ", risk_threshold*100, "th percentile"), collapse = "")
low_risk_cat <- paste(c("Risk 0-", risk_threshold*100, "th percentile"), collapse = "")
...
list_plots_unadjusted[[1]] <- ggsurvplot(
  ...
  xlab = "",
  ylab = "40% decline eGFR / ESKD",
  title = paste0(c("Microalbuminuria and ", low_risk_cat), collapse=""),
  # subtitle = paste0(c(
  #   
  #   # "Mean background risk ", round(obs_v_pred40[obs_v_pred40$risk_group == alb_and_low_risk
  #   #                                             ,]$surv*100, 1), 
  #   # "% (predicted ", round(obs_v_pred40[obs_v_pred40$risk_group == alb_and_low_risk
...
list_plots_unadjusted[[3]] <- ggsurvplot(
  ...
  # subtitle = paste0(c(
  #   "Mean background risk ", round(obs_v_pred40[obs_v_pred40$risk_group == alb_and_high_risk
  #                                               ,]$surv*100, 1), 
  #   "% (predicted ", round(obs_v_pred40[obs_v_pred40$risk_group == alb_and_high_risk
...
list_plots_unadjusted[[2]] <- ggsurvplot(
  ...
  # subtitle = paste0(c(
  #   "Mean background risk ", round(obs_v_pred40[obs_v_pred40$risk_group == nonalb_low_risk
  #                                               ,]$surv*100, 1), 
  #   "% (predicted ", round(obs_v_pred40[obs_v_pred40$risk_group == nonalb_low_risk
  #                                       ,]$mean_predicted_background_risk*100, 1), "%)"),collapse="")
) 
...
list_plots_adjusted[[1]] <- ggsurvplot(
  ...
  subtitle = paste0(c(
    ...
    # "% (predicted ", round(obs_v_pred40[obs_v_pred40$risk_group == alb_and_low_risk
    #                                     ,]$mean_predicted_background_risk*100, 1), "%),
    low_risk_cat, "; 3-year NNT ",
    round(1/(summary(surv_alb_lowrisk, times = 3)$surv[2]-summary(surv_alb_lowrisk, times = 3)$surv[1]), 0)
    # ,
    # ", 95% CI ", 
    # round(1/(summary(surv_alb_lowrisk, times = 3)$upper[2]-summary(surv_alb_lowrisk, times = 3)$upper[1])
    #       , 0),
    # "-",
    # round(1/(summary(surv_alb_lowrisk, times = 3)$lower[2]-summary(surv_alb_lowrisk, times = 3)$lower[1])
    #       , 0)
  ),collapse="")
)
...
list_plots_adjusted[[3]] <- ggsurvplot(
  ...
  subtitle = paste0(c(
    ...
    # "% (predicted ", round(obs_v_pred40[obs_v_pred40$risk_group == alb_and_high_risk
    #                                     ,]$mean_predicted_background_risk*100, 1), "%), 
    high_risk_cat, "; 3-year NNT ",
    round(1/(summary(surv_alb_highrisk, times = 3)$surv[2]-summary(surv_alb_highrisk, times = 3)$surv[1]), 0)
    # ,
    # ", 95% CI ", 
    # round(1/(summary(surv_alb_highrisk, times = 3)$upper[2]-summary(surv_alb_highrisk, times = 3)$upper[1])
    #       , 0),
    # "-",
    # round(1/(summary(surv_alb_highrisk, times = 3)$lower[2]-summary(surv_alb_highrisk, times = 3)$lower[1])
    #       , 0)
  ),collapse="")
) 
...
list_plots_adjusted[[2]] <- ggsurvplot(
  ...
  subtitle = paste0(c(
    ...
    # ,
    # ", 95% CI ", 
    # round(1/(summary(surv_noalb_lowrisk, times = 3)$upper[2]-summary(surv_noalb_lowrisk, times = 3)$upper[1])
    #       , 0),
    # "-",
    # round(1/(summary(surv_noalb_lowrisk, times = 3)$lower[2]-summary(surv_noalb_lowrisk, times = 3)$lower[1])
    #       , 0)
  ),collapse="")
) 
