...
cutoff1 <- cohort %>% filter(risk_group %in% c("uACR 3-30mg/mmol")) %>% .$ckdpc_50egfr_sglt2i_benefit %>% quantile(0.90) %>% as.numeric()

cutoff1_equivalent_40egfr_score <- cohort %>% filter(risk_group %in% c("uACR 3-30mg/mmol")) %>% .$ckdpc_40egfr_score %>% quantile(0.90) %>% as.numeric()

cutoff2 <- cohort %>% filter(!risk_group %in% c("uACR ≥30mg/mmol")) %>% .$ckdpc_50egfr_sglt2i_benefit %>% quantile( 
  1 - (cohort %>% filter(risk_group %in% c("uACR 3-30mg/mmol")) %>% nrow() / cohort %>% filter(!risk_group %in% c("uACR ≥30mg/mmol")) %>% nrow())) %>% as.numeric()

cutoff2_equivalent_40egfr_score <- cohort %>% filter(!risk_group %in% c("uACR ≥30mg/mmol")) %>% .$ckdpc_40egfr_score %>% quantile( 
  1 - (cohort %>% filter(risk_group %in% c("uACR 3-30mg/mmol")) %>% nrow() / cohort %>% filter(!risk_group %in% c("uACR ≥30mg/mmol")) %>% nrow())) %>% as.numeric()

cohort <- cohort %>% mutate(
  treat_guideline = ifelse(risk_group == "uACR <3mg/mmol", F, T),            # everyone current guidelines recommend treating (uACR cutoff)
  treat_model1 = ifelse(ckdpc_50egfr_sglt2i_benefit > cutoff1, T, F),        # anyone with benefit above upper quartile of those currently recommended treatment in 3-30mg/mmol
  treat_model2 = ifelse(ckdpc_50egfr_sglt2i_benefit > cutoff2, T, F)         # treating same proportion of people as guidelines would recommend but based on risk-score rather than uACR cutoff
) %>%
  mutate(
    treat_guideline = ifelse(risk_group == "uACR ≥30mg/mmol", TRUE, treat_guideline),
    treat_model1 = ifelse(risk_group == "uACR ≥30mg/mmol", TRUE, treat_model1),
    treat_model2 = ifelse(risk_group == "uACR ≥30mg/mmol", TRUE, treat_model2)
  )
...
benefit_histogram <- ggplot(cohort %>% filter(!risk_group == "uACR ≥30mg/mmol") %>%
...
obs_v_pred_for_plot <- cohort %>%
  filter(!risk_group == "uACR ≥30mg/mmol") %>%
...
#cutoff 1: top decile of predicted benefit amongst those with uACR <30 that guidelines recommend treating.
...
#cutoff 2: same number of people treated as per guidelines except based on risk rather than albuminuria 3 mg/mmol threshold
...
grid.arrange(arrangeGrob(p2[[1]][["plot"]] + theme(legend.position=c(0.5, 0.7), legend.title=element_blank(), legend.text=element_text(size=16, face="bold"), plot.title=element_text(face="bold", hjust=0.5, margin=margin(b=5, t=12), size = 22), plot.subtitle=element_text(hjust=0.5, margin=margin(b=-25), size = 16), axis.title.y=element_text(vjust=-0.3), plot.margin=margin(l=5, r=5)) + guides(colour = guide_legend(nrow = 1)),
                         ...
                         p2[[4]][["plot"]] + theme(legend.position="none", plot.title=element_text(face="bold", hjust=0.5, margin=margin(b=5, t=12), size = 22), plot.subtitle=element_text(hjust=0.5, margin=margin(b=-25), size = 16), axis.title.y=element_text(vjust=-0.3), plot.margin=margin(l=5, r=5)), ncol=2, nrow=2, widths=c(1,1)))
dev.off()
...
nnt_median_guideline_not_recommended <- cohort %>% filter(risk_group == "uACR <3mg/mmol") %>% summarise(median_benefit = median(survdiff_ckd_egfr50)) %>% mutate(nnt = 1/median_benefit) %>% select(nnt) %>% as.numeric()
nnt_median_model1_recommended <- cohort %>% filter(treat_model1 == T) %>% summarise(median_benefit = median(survdiff_ckd_egfr50)) %>% mutate(nnt = 1/median_benefit) %>% select(nnt) %>% as.numeric()
nnt_median_model1_not_recommended <- cohort %>% filter(treat_model1 == F) %>% summarise(median_benefit = median(survdiff_ckd_egfr50)) %>% mutate(nnt = 1/median_benefit) %>% select(nnt) %>% as.numeric()
...
nnt_mean_guideline_not_recommended <- cohort %>% filter(risk_group == "uACR <3mg/mmol") %>% summarise(mean_benefit = mean(survdiff_ckd_egfr50)) %>% mutate(nnt = 1/mean_benefit) %>% select(nnt) %>% as.numeric()
nnt_mean_model1_recommended <- cohort %>% filter(treat_model1 == T) %>% summarise(mean_benefit = mean(survdiff_ckd_egfr50)) %>% mutate(nnt = 1/mean_benefit) %>% select(nnt) %>% as.numeric()
nnt_mean_model1_not_recommended <- cohort %>% filter(treat_model1 == F) %>% summarise(mean_benefit = mean(survdiff_ckd_egfr50)) %>% mutate(nnt = 1/mean_benefit) %>% select(nnt) %>% as.numeric()

# 
# #### counterfactual observed events if no one treated
# events_none_treated <- (1-obs_model1_Y_DPP4) * nrow(cohort_model1_Y) + (1-obs_model1_N_DPP4) * nrow(cohort_model1_N)
# events_none_treated / n.imp
# #### counterfactual observed events if everyone treated
# events_everyone_treated <- (1-obs_model1_Y_SGLT2) * nrow(cohort_model1_Y) + (1-obs_model1_N_SGLT2) * nrow(cohort_model1_N)
