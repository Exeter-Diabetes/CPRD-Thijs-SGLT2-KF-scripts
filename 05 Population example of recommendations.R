...
cohort <- cohort %>% mutate(
  newindication = ifelse(
    macroalbuminuria == T, "uACR ≥30mg/mmol", ifelse(
      egfr_below_60 == T & microalbuminuria == T,
    "eGFR <60mL/min/1.73m2,\nuACR 3-30mg/mmol", ifelse(
      egfr_below_60 == T & microalbuminuria == F,
      "eGFR <60mL/min/1.73m2,\nuACR <3mg/mmol", ifelse(
       uacr < threshold_1, "eGFR ≥60mL/min/1.73m2,\nuACR 3-30mg/mmol,\nCKD-PC risk score 0-90th percentile", 
       "eGFR ≥60mL/min/1.73m2,\nuACR 3-30mg/mmol,\nCKD-PC risk score ≥90th percentile")))),
  newindication = newindication %>%
    factor(levels = c("eGFR ≥60mL/min/1.73m2,\nuACR 3-30mg/mmol,\nCKD-PC risk score ≥90th percentile", 
                      "eGFR ≥60mL/min/1.73m2,\nuACR 3-30mg/mmol,\nCKD-PC risk score 0-90th percentile", 
                      "eGFR <60mL/min/1.73m2,\nuACR <3mg/mmol", 
                      "eGFR <60mL/min/1.73m2,\nuACR 3-30mg/mmol", 
                      "uACR ≥30mg/mmol"))
...
ggplot(test2, aes(x = x, 
                  ...
                  label = fct_rev(node))) +
  theme_sankey() +
  scale_fill_manual(values = c("#E69F00", "grey", "#D55E00", "#D55E00", "#D55E00", "#D55E00")) + 
